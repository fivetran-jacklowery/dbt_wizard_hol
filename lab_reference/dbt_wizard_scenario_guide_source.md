...
| Step | User Prompt | What It Demonstrates | dbt Wizard Functionality |
| :---- | :---- | :---- | :---- |
| 0 | (Run in the terminal, not in Wizard, `cd /Users/youraccount/summit2026/dbt_wizard_hol`And then Run `dbt run --select stg_products+` | Reproduces the failure live so the rest of the workflow is grounded in a real error message, not a hypothetical. | terminal — dbt run |
