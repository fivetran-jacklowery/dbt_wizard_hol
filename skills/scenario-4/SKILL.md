...
Exercises `status`, `dbt_show` against `run-results`, and error parsing. We start from the failure, not from a hunch. The user does not need to scroll through stack traces in the terminal — dbt Wizard surfaces the model name, the failing column, and the source it traces back to.
...
When the current schema comes back, the user reads it and spots that `[OLD_COLUMN]` is gone and `[NEW_COLUMN]` is new. Confirm with the user in one line: the rename is `[OLD_COLUMN]` → `[NEW_COLUMN]`. Do not let the user move on until they've named both columns out loud — guessing the new column name from context is the #1 way this fix goes sideways.
...
- **Path A (brand → brand_name):** `stg_products.sql` plus 8 downstream files — 4 intermediates and 4 marts. If only 2–3 files come back, dbt Wizard missed the marts; push back.
- **Path B (status → order_status):** `stg_orders.sql`, the column-test entry in `_staging__models.yml`, and ~7 downstream files including `int_orders_enriched`, `int_daily_revenue`, `int_customer_order_summary`, and `fct_orders`. Wide list — that's the point of picking this path.
- **Path C (customer_type → segment):** `stg_customers.sql`, the column-test entry in `_staging__models.yml`, and 5 downstream files (1 intermediate + 4 marts). Compact list.
...
- The output still has a column named `[OLD_COLUMN]` (the alias survived) — values look sane.
